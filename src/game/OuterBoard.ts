...
export class OuterBoard extends Board {
  constructor() {
    super();
    for (
      let rowIdx = Coordinates.MIN_VALUE;
      rowIdx <= Coordinates.MAX_VALUE;
      ++rowIdx
    ) {
      this.fields[rowIdx] = [];
      for (
        let colIdx = Coordinates.MIN_VALUE;
        colIdx <= Coordinates.MAX_VALUE;
        ++colIdx
      ) {
  ...
  getBoard(coordinates: Coordinates): InnerBoard {
    return this.fields[coordinates.outerRow][
      coordinates.outerCol
    ] as InnerBoard;
  }
